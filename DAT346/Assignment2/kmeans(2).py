...
def kmeans(k, data, workers, nr_iter=100):
    ...
    for j in range(nr_iter):
        ...
        # list_of_assignments = [p.get() for p in process_objects]

        list_of_assignments = process_objects

        assignment_flat_list = [item for sublist in list_of_assignments for item in sublist]
        ...
        for i in range(N):
            cluster = assignment_flat_list[i][0]
            c[i] = cluster
            dist = assignment_flat_list[i][1]
...
def computeClustering(args):
    ...
    logging.warning("Clustering complete in %3.2f [s]" % (end_time - start_time))
